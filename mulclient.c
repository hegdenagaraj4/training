...
int main(int argc, char *argv[])
{
    ...
    {
         g_no_of_clients++;
         debug("number of clients added is %d",g_no_of_clients);
    }
    ...
    while(1)
    {
        ...
        debug("Received %d bytes from %s: ", msgrecv_len,
        inet_ntoa(from_addr.sin_addr));
        debug("%s", message_received);
    }
